<div class="main-sidebar sidebar-style-2">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            <a href="index.html">SAID</a>
        </div>
        <div class="sidebar-brand sidebar-brand-sm">
            <a href="index.html">SA</a>
        </div>
        <ul class="sidebar-menu">
            <li class="menu-header">Dashboard</li>
            <li class="dropdown">
                <a href="/home" class="nav-link"><i class="fas fa-fire"></i><span>Dashboard</span></a>
            </li>

            <li class="menu-header">MAIN MENU</li>
            <li class="dropdown">
                <a href="/datauser" class="nav-link "><i class="fas fa-th-large"></i> <span>Data User</span></a>
            </li>
            <li class="dropdown">
                <a href="#" class="nav-link "><i class="far fa-file-alt"></i> <span>Pengajuan Surat</span></a>

            </li>
            <li class="dropdown">
                <a href="/informasi" class="nav-link"><i class="fas fa-map-marker-alt"></i>
                    <span>Informasi</span></a>

            </li>

            <li class="dropdown">
                <a href="#" class="nav-link "><i class="fas fa-plug"></i> <span>Aspirasi</span></a>

            </li>
            {{-- <li class="dropdown">
                <a href="#" class="nav-link has-dropdown"><i class="fas fa-plug"></i> <span>Modules</span></a>
                <ul class="dropdown-menu">
                    <li><a class="nav-link" href="#">Calendar</a>
                    </li>
                    <li><a class="nav-link" href="#">ChartJS</a>
                    </li>
                    <li><a class="nav-link" href="#">DataTables</a></li>
                    <li><a class="nav-link" href="#">Flag</a></li>
                </ul>
            </li> --}}
        </ul>

        {{-- <div class="mt-4 mb-4 p-3 hide-sidebar-mini">
            <a href="#" class="btn btn-primary btn-lg btn-block btn-icon-split">
                <i class="fas fa-rocket"></i> Documentation
            </a>
        </div> --}}
    </aside>
</div>
